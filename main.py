...
def submit_to_ghl_form(first_name, last_name, phone, email, about_case):
    with sync_playwright() as p:
        ...
        # Correct way to get frame object
        frame = page.frame(name="inline-UQVFhuQiNCfdJTydbFrm")
        if not frame:
            # fallback if name doesn't work, try url
            for f in page.frames:
                if "leadconnectorhq.com/widget/form/UQVFhuQiNCfdJTydbFrm" in (f.url or ""):
                    frame = f
                    break
            if not frame:
                browser.close()
                raise Exception("Could not locate iframe for the GHL form")
